...
if [ "$1" = "run" ]; then
	...
	docker run --rm --runtime=nvidia -it \
	    ...
	    --volume=`pwd`/front_images:/root/input_data:rw \
	    --volume=`pwd`/output_data:/root/output_data:rw \
	    ${user}/${containerName}:${containerTag} /bin/bash -c "cd /root/camodocal/build/bin && ./$2 && mv *.yaml *.dat /root/output_data && chmod a+rw /root/output_data/*"
